  ...
  const Eigen::MatrixXi& F)
{
  ...
  get_scale_and_shift_to_fit_mesh(V,F,camera_zoom,camera_translation);
  ...
  const Eigen::MatrixXd& V)
{
  ...
  get_scale_and_shift_to_fit_mesh(V,camera_zoom,camera_translation);
  // Rather than crash on empty mesh...
  if(V.size() > 0)
  {
    object_scale = (V.colwise().maxCoeff() - V.colwise().minCoeff()).norm();
  ...
  bool update_matrices)
{
  ...
  if(update_matrices)
  {
    ...
    view = view
      * (trackball_angle * Eigen::Scaling(camera_zoom)
